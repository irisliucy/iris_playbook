"""Stacked Denoising Convolutional Autoencoder Implementation"""
...
class NNLayer:
    ...
    def encode(self, input_tensor, use_variables=False):
        ...
        if use_variables: # use_variables = true during fine tunning, dynamic weight is used
            return self.activate(tf.add(tf.nn.conv2d(
                input_tensor, self._weights, strides=[1, 2, 2, 1], padding='SAME'), self._biases))
        else:
            return self.activate(tf.add(tf.nn.conv2d(
                input_tensor, self.weights, strides=[1, 2, 2, 1], padding='SAME'), self._biases))
...
class SCDAutoencoder:
    """A stacked denoising convoulutional autoencoder."""
    ...
    def pretrain_layer(self, depth, batch_generator):
        ...
        with tf.name_scope(hidden_layer.name):
            ...
            with tf.name_scope("x_values"):
                x_original = tf.placeholder(tf.float32, shape=[None, self.input_dim]) # input to the network
                x_latent = self.get_encoded_input(x_original, depth, use_variables=False)
                # ensure 2-d is converted to square tensor.
                if len(x_latent.get_shape()) == 2:
                    x_dim = np.sqrt(x_latent.get_shape().as_list()[1])
                    if x_dim != int(x_dim):
                        raise ValueError('Unsupported input dimensions')
                    x_dim = int(x_dim)
                    x_tensor = tf.reshape(
                        x_latent, [-1, x_dim, x_dim, 1])
                ...
                if self.corruption: #
                    x_latent = corrupt(x_latent, corruption_level=self.noise)

            # Encoder's weight and biases
    ...
    def finetune_parameters_gen(self, xy_train_gen, output_dim):
        ...
        with tf.name_scope("finetuning"):
            ...
            with tf.name_scope("softmax_variables"):
                W = weight_variable(self.output_dim, output_dim, name="weights") # output dimension of the autoencoder stack as the input for finetunning model
            ...
            with tf.name_scope("cross_entropy"):
                cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_actual, logits=y_logits))
